...
for building in building_columns:
    print(f"\nTraining Linear Regression model for {building}...")

    # Drop NaNs
    df_building = df.dropna(subset=[building] + base_features)

    # Inject area as constant feature
    area = area_map[building]
    df_building["area"] = area  # broadcast constant value
    ...
    # Save model
    filename = building.replace(", ", "_").replace(" ", "") + "_linreg_model.pkl"
